...
static lv_obj_t * DevMode_switch = NULL;
...
void LifeguardSetupTileSetup(uint32_t tileNum)
{
    ...
    lv_obj_t * header = wf_add_settings_header(lifeguardSetupTile, "lifeguard setup", LifeguardSetupExitEvent);
    ...
    // lv_obj_t * lifeGuardTempMax_obj = CreateListObject(lifeguardSetupTile, lifeGuardEmergencyTime_obj);
    ...
    // lv_textarea_set_cursor_hidden( lifeguardTempMin_textfield, true);
